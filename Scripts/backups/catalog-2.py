...
def arrange_catalog(init_catalog, vndr):

	print("=== Arrange Catalog ===\n")

	print("Length of Catalog: " + str(len(init_catalog)) + "\n")

	input_collections = isolate_collections(init_catalog) # now we have an array of isolated collections

	products = []

	# loop through collections in catalog, rather than rows in catalog, b/c we are creating 1 product per collection. 1 product per collection b/c it is a room set.
	for collection_idx in range(len(input_collections)):
		init_collection = input_collections[collection_idx] # now we have a single sample collection
		...
		product_type = product.set_type(init_collection)
		if product_type == "bedroom sets":

			# product info to compute
			product_handle = product.set_handle(init_collection)
			...
			product_title = product.set_title(init_collection)
			...
			variant_items = collection.isolate_variant(init_collection, product_type)

			variant_sku = product.set_sku(variant_items)
			variant_price = product.set_price(variant_items)
			variant_inv_tracker = "shopify"
			option1_name = product.set_option_name(product_type)
			option1_value = product.set_option_value(product_type)

			final_catalog_row = "\"" + product_handle + "\",\"" + product_command + "\",\"" + product_title + "\",\"" + product_description + "\",\"" + product_vendor + "\",\"" + product_type + "\",\"" + product_tags + "\",\"" + product_publish_status + "\",\"" + product_publish_scope + "\",\"" + product_img_src + "\",\"" + variant_sku + "\",\"" + variant_price + "\",\"" + variant_inv_tracker + "\",\"" + option1_name + "\",\"" + option1_value + "\""
...
def isolate_collections(catalog):
...
def isolate_catalog_field(catalog, field_title):
	...
	for row_idx in range(len(catalog)):
		...
		cell3 = init_catalog_row[2] # description for item rows, price for parent row
		...
		if field_title == "parent sku":
			...
			if determine_new_collection(cell1, cell3): # if parent or first item in collection
...
def determine_new_collection(value1, value3):
	...
	if simple_value1.isupper():
		new_collection = False
	elif re.search(item_key, simple_value3):
		new_collection = False
	else:
		new_collection = True
