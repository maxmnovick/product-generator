...
def divide_collections(collections):
	# get indexes of where collections split and use later to set loop iterations for linked products
	product_counts = []

	previous_collection_handle = current_collection_handle = ""

	for row_idx in range(len(collection)):
		#print("original collection row: " + collection[row_idx])

		init_collection_info = collection[row_idx].split("\",")
		for info in range(len(init_collection_info)):
			init_collection_info[info] = init_collection_info[info].replace("\"", "")

		current_collection_handle = init_collection_info[1]

		if previous_collection_handle == "":
			pass

		elif not current_collection_handle == previous_collection_handle:
			# different collection, so record current index as number of products in previous collection (first index of new, current collection)
			previous_collection_length = row_idx - np.sum(product_counts)
			product_counts.append(previous_collection_length)
...
input_collections = read_collection(category_filename) # array of strings, where each string is csv row of collection where commas separate data fields
#print("input_collections: " + str(input_collections) + "\n")

linked_product_counts = divide_collections(input_collections)

# for all collections, as determined by dividing collections by handle
for collection_idx in range(len(linked_product_counts)):

	input_linked_products = isolate_linked_products(linked_product_count)
	...
	for row_idx in range(linked_product_count):

		init_collection_info = current_collection[row_idx].split("\",")
		for info in range(len(init_collection_info)):
			init_collection_info[info] = init_collection_info[info].replace("\"", "")
